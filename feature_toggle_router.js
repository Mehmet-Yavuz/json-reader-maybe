const SharedUtilities = require("../../shared/shared_utilities");
...
class FeatureToggleRouter {
	...
	validatePassPhrase(passPhrase, description = undefined) {
		if ((typeof passPhrase !== "string") || (passPhrase.trim() === "")) {
			throw new Error(`Pass phrase ${description ? (`for ${description}`) : ""} needs to be a non-empty string.`);
		}
	...
	getFeatureMetadata(featureName, configuration = this.activeConfiguration) {
		...
		while (pathSeparatorIndex >= 0) {
			...
			if (Object.prototype.hasOwnProperty.call(parentConfiguration, parentName)) {
		...
		if (Object.prototype.hasOwnProperty.call(parentConfiguration, remainingPath)) {
	 ...
	 */
	getValue(featureName, defaultValue = undefined, configuration = this.activeConfiguration) {
	 ...
	 */
	getTitle(featureName, defaultValue = undefined, configuration = this.activeConfiguration) {
		const featureMetadata = this.getFeatureMetadata(featureName, configuration);
		if (featureMetadata !== undefined) {
	 ...
	 * @returns {Object} The title of the feature.
	 */
	getDescription(featureName, defaultValue = undefined, configuration = this.activeConfiguration) {
		const featureMetadata = this.getFeatureMetadata(featureName, configuration);
		if (featureMetadata !== undefined) {
			if (Object.prototype.hasOwnProperty.call(featureMetadata, "ftrDescription")) {
	...
	async isEncrypted(configuration) {
		if (Object.prototype.hasOwnProperty.call(configuration, "ftrLicense")) {
			...
			if (this.deployment === undefined) {
				throw new Error("The deployment needs to be set before decrypting a configuration.");
			}
			if (this.encryptionPrivateKey2 === undefined) {
				throw new Error("The encryption private key and encryption pass phrase need to be set before decrypting a configuration.");
			}
	...
	async isSigned(configuration) {
		if (Object.prototype.hasOwnProperty.call(configuration, "ftrSignature")) {
			...
			if (this.deployment === undefined) {
				throw new Error("The deployment needs to be set before checking the signature of a configuration.");
			}
			if (this.signaturePublicKey2 === undefined) {
				throw new Error("The signature public key needs to be set before checking the signature of a configuration.");
			}

			// The configuration is signed. Check the signature.
			const ftrSignature = configuration.ftrSignature;
			if (Object.prototype.hasOwnProperty.call(configuration, "ftrDeployment")) {
				...
				if (ftrDeployment != this.deployment) {
					throw new Error(`Configuration contains the wrong ftrDeployment value: '${ftrDeployment}' instead of required '${this.deployment}'.`);
				}
				...
				const message = await openpgp.createMessage({ text: JSON.stringify(unsignedConfiguration) });
				...
				throw new Error("Configuration contains a ftrSignature but no ftrDeployment.");
			}
		}
	...
	async isLicensed(configuration) {
		return (await this.isEncrypted(configuration) !== false) || await this.isSigned(configuration);
	}
	...
	async loadConfiguration(source, configuration) {
		...
		try {
			...
			await this.loadFeature(source, configurationToParse, this.activeConfiguration, configurationStatus, isLicensed);
	 ...
	 */
	async loadFeature(source, parentFeature, activeParentFeature, configurationStatus, isLicensed) {
		for (const [featureName, feature] of Object.entries(parentFeature)) {
			...
			if (!Object.prototype.hasOwnProperty.call(activeParentFeature, featureName)) {
			...
			const ftrLocked = Object.prototype.hasOwnProperty.call(activeFeature, "ftrLocked") && (activeFeature.ftrLocked == true);

			const ftrLockedValues = activeFeature.ftrLockedValues;

			// Update feature locked values (ftrLockedValues), if it is specified.
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrLockedValues")) {
				...
				if (!ftrLocked || isLicensed || (Array.isArray(ftrLockedValues) && SharedUtilities.isSubset(ftrLockedValues, newFtrLockedValues))) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrValue")) {
				// Get the actual value (ftrValue) from the feature metadata.
				...
				if (!ftrLocked || isLicensed || (Array.isArray(ftrLockedValues) && ftrLockedValues.includes(newFtrValue))) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrRange")) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrValidationRegex")) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrTitle")) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrDescription")) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrOptInEnabled")) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrOptInValues")) {
			...
			if (Object.prototype.hasOwnProperty.call(newFeatureMetadata, "ftrLocked")) {
			...
			if (typeof feature === "object") {
				await this.loadFeature(source, feature, activeFeature, configurationStatus, isLicensed);
			}
		}
	}
	...
	buildConfigurationWithFeature(featureName, featureMetadata) {
		...
		const configuration = featureName.split(".").reduceRight((previousValue, currentValue) => {
			return {
	...
	async setValue(source, featureName, featureValue) {
		...
		const configuration = this.buildConfigurationWithFeature(featureName, featureMetadata);
	...
	async setLocked(source, featureName, locked) {
		if (typeof locked !== "boolean") {
			throw new Error("The locked value should be a boolean 'true' of 'false'.");
		}
